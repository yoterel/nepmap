...
class VanillaNeRFRadianceField(nn.Module):
    ...
    def forward(self, x, condition=None, texture_ids=None, light_field=None, calc_norms=False, cur_step=None):
        if calc_norms:
            x.requires_grad = True
        pos_x = self.posi_encoder(x)
        if condition is not None:
            condition = self.view_encoder(condition)
        rgb, sigma = self.mlp(pos_x, condition=condition)
        if calc_norms:
            # x.requires_grad = True
        ...
        if light_field is None:
            ...
            for i, projector in enumerate(light_field["projectors"]):
                ...
                visible_texture = sampled_texture * projector["amp"]
            final_color = torch.sigmoid(rgb) * visible_texture
        return final_color, None, F.relu(sigma), None, normals, None, None, None, None, visible_texture
